...
> [!TIP]
> **关于 NCBI API Key**
> PubMed 的 E-utilities API 对非注册用户有速率限制（3次请求/秒）。
> 建议您注册 NCBI 账号并申请 API Key（免费），配置后可将速率限制提升至 **10次请求/秒**，显著加快文献抓取速度。
...
# 或 npm run dev
```
此命令将启动开发服务器并生成 `build/chrome-mv3-dev` 目录。在 Chrome 浏览器的“扩展程序”页面开启“开发者模式”，点击“加载已解压的扩展程序”，选择该目录即可。
...
# 或 npm run package
```
此命令将在 `build` 目录下生成用于上传 Chrome Web Store 的 `.zip` 文件。

## 最新功能 (v1.1)

-   **🚀 流式响应 (Streaming)**: 无论是生成研究计划、评估文献还是撰写最终报告，AI 的输出都会实时逐字显示。用户可以立刻看到 AI 的“思考过程”，彻底告别长任务的等待焦虑。
-   **⚙️ 多模型配置管理**: 支持同时配置多套 AI 模型参数（如 Gemini Pro, GPT-4o 等），并为不同配置设置独立的 API Key 和速率限制。用户可以随时在侧边栏一键切换当前使用的配置。
-   **⚡️ 连接测试与速率控制**: 在设置页面即可测试 API 连接通断。新增自定义抓取速率限制功能，帮助用户在“速度”与“反爬虫风控”之间找到最佳平衡。
...
2.  **[插件] 任务分解 (LLM调用 #1: 研究策略师)**: 后台脚本接收到主题，调用LLM API，**流式生成**一个包含多个子问题和对应搜索关键词的研究计划草案。
3.  **[用户] 协商与确认**:
...
2.  **[插件] 智能信源评估 (LLM调用 #2: 文献审阅人)**:
    * 收集到所有摘要后，后台脚本再次调用LLM，指令其根据研究计划为每篇摘要的相关性打分(1-10分)，并提供一句话评审理由。**评分过程实时流式展示**。
3.  **[用户] 筛选信源**:
...
4.  **[插件] 深度阅读与综述生成 (LLM调用 #3: 研究综述员)**:
    ...
    * 后台脚本将所有全文内容和研究计划一同发送给LLM，指令其撰写一份全面、客观、结构化的文献综述报告，并正确引用来源（`[PMID:XXXXXX]`）。**报告内容实时流式生成，所见即所得**。
...
-   ✅ **里程碑 5: 体验优化 (v1.1)** - 实现了全链路流式响应 (Streaming)、多模型配置管理、连接测试与速率限制，大幅提升了用户体验和工具的灵活性。
-   ➡️ **里程碑 6: 下一步计划** - 引入更多 Web 搜索源（如 Google Search, Tavily）以补充 PubMed 之外的上下文信息；支持导出 PDF 报告。